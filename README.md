...
- **출장 구간 선택**: 종일 / 오전만 / 오후만 — 반나절 출장이면 나머지 반나절은 평소 근무
- **출장 인정(관리자 승인)**: 승인한 출장은 그 구간만큼(종일 8h · 오전/오후 4h) 근로시간으로 인정해
  근로부족·조기퇴근·코어타임 미충족 판정에서 제외. 인정은 소정근로를 넘지 않음(초과근무로 부풀지 않음).
  관리자는 [승인] 탭의 *출장 인정 대기* 목록이나 직원 근태 수정 화면에서 출장을 추가·변경·인정할 수 있음
